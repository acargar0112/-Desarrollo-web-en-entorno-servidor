...
def agregar_artista(evento, nombre, *temas):
    ...
    except Comprobar_artista as e:
        print(f"ERROR: {e}")
    except Artista_existente as i:
        print(f"ERROR: {i}")
    else:
...
def programar_combate(evento, titulo, luchador1, luchador2, rounds, peso):
    ...
    except Combate_rounds as e:
        print(f"ERROR: {e}")
    except Combate_existente as i:
        print(f"ERROR: {i}")
    else:
...
def informe_completo(evento):
    ...
    luchadores_no_repe = set()
    for i in evento["combates"].values():
        for j in i["luchadores"]:
            luchadores_no_repe.add(j.nombre)
    luchadores_no_repe = list(luchadores_no_repe)
    for i in luchadores_no_repe:
        (print)(f"- {i}")
    print("")
...
def main():
    while True:
        ...
        match opcion:
            case 1:
                tipo = input("Introduce un tipo de entrada(general-premium-vip): ").lower().strip()
                unidades = int(input("Introduce el número de entradas: "))
                print(vender_entradas(evento,tipo,unidades))
            case 2:
                temas = set()
                nombre = input("Introduce el nombre del artistas: ").lower().capitalize().strip()
                tema = input("Introduce las canciones(ENTER para terminar de introducir canciones.): ").lower().capitalize().strip()
                temas.add(tema)
                while tema != "":
                    tema = input("Introduce las canciones(ENTER para terminar de introducir canciones.): ").lower().capitalize().strip()
                    if tema == "":
                    ...
                    temas.add(tema)
                temas = list(temas)
                print(agregar_artista(evento,nombre, *temas))
            case 3:
                titulo = input("Introduce el título de la pelea: ").lower().capitalize().strip()
                l1 = input("Introduce el primer luchador: ").lower().capitalize().strip()
                l2 = input("Introduce el segundo luchador: ").lower().capitalize().strip()
                rounds = int(input("Introduce el numero de rounds: "))
                peso = input("Introduce la categoría del peso: ").lower().capitalize().strip()
                print(programar_combate(evento,titulo,l1,l2,rounds,peso))
            case 4:
                titulo = input("Introduce el título de la pelea: ").lower().capitalize().strip()
                ganador = input("Introduce el ganador: ").lower().capitalize().strip()
                print(cerrar_resultado(evento,titulo,ganador))
            case 5:
